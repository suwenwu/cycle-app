...
- ${ctx.recommendation.intensityCaps.z3z4 ? '本周禁止 3区/4区 灰色区域训练，只做 1区/2区 或高强度训练' : '允许适量 3区/4区，但优先级低于 1区/2区 和高强度训练'}
...
const STYLE_PROMPTS: Record<TrainingStyle, string> = {
  polarized: `### 核心原则（基于 Seiler/Stöggl/Hebisz 极化训练研究）
- 80/20 两极化：低强度有氧占 ≥80% 训练量，高强度只做 6区纯厌氧，0% 灰色区域
- 轻松日严格低于 VT1（≤75% FTP，自主神经零压力），高强度日 ≥120% FTP（最大化适应性信号）
- 周结构：状态绿灯 2 次高强度，黄灯 1 次，红灯只做低强度

### 强度分布（Stöggl & Sperlich 2014, Neal 2013）
- 1区/2区：≥ 80%
- 6区：≤ 20%
- 3区/4区/5区/7区：0%（极化训练不做中间强度，也不做纯冲刺——那是专项技术，不是体能训练）

### 低强度课（Seiler VT1 阈值理论）
- 功率 ≤ 75% FTP，持续 60-180 分钟
- 长距离日（≥120min）可嵌入 6-8 次 × 5 秒全力冲刺，相隔 ≥20 分钟（Stöggl POL 方案，低疲劳神经肌肉维护）
- 禁止持续 >80% FTP 的输出

### Z6 厌氧间歇（120-150% FTP，基于 Hebisz 2024 SIT / FasCat / Coggan）
A) SIT 冲刺间歇：8-12 × 30 秒全力。分 2-3 组（4 次/组），组内 90 秒轻松恢复，组间 15-25 分钟 Z2
B) 标准厌氧：4-6 × 45-90 秒 @120-150% FTP，组间完全恢复 3-5 分钟
C) 分组 30/30 间歇：每组 8 × (30s ON @130-150% / 30s OFF 轻松)，组间大休息 5-8 分钟，共 2-3 组
- 单次有效时间：8-12 分钟
- 要求每组都能维持目标功率，做不到就减组数或降功率

### 恢复规则（Z6 的核心是恢复质量）
- 工作/恢复 ≥ 1:1（30s → ≥30s）；追求绝对功率时 ≥ 1:3
- 组间大休息（5-8 分钟）保证下一组质量，不是装样子
- 缩短恢复 = 改变训练目标（变成耐乳酸，不是厌氧功率）
- 高强度步骤给单个确切功率值；恢复步功率 ≤50% FTP
- 两次高强度日隔 ≥1 天 Z2 或休息；一周 2 次则用不同方案（如 A+B、B+C）

### 课表结构
- 热身：15-20 分钟渐进 ramp 50%-75%，含 3-4 次 5 秒加速（唤醒神经肌肉）
- 主 set：按上面方案选 1 种，同一天只做一种
- 放松：10-15 分钟 ramp 75%-50%

### 安全约束
- 红灯/TSB<-10/HRV<基线85%/睡眠<6h → 取消高强度，改 Z1/Z2 等时长
- 本周 ≥2 次高强度后不再安排
- 高强度日功率绝不低于 120% FTP`,
      ...
      "workoutDoc": {
        ...
        "steps": [
          ...
          {"reps": 6, "text": "Main Set", "steps": [
             {"power": {"value": 336, "units": "w"}, "duration": 60, "text": "6区 336W"},
             {"power": {"value": 140, "units": "w"}, "duration": 120, "text": "恢复 140W"}
          ]},
